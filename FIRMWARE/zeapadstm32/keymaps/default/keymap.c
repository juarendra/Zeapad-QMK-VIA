 ...
 */
[_BASE] = LAYOUT_via(
    KC_P7,    RP_TRIGGER,           	    HLD_TRIGGER,               TO(1),    
    KC_P4,    LCTL(KC_C),                   LCTL(KC_V),              KC_ESC,      
    KC_P1,    LCTL(KC_A),                   KC_Z,                    QK_LOCK,       
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
		...
		case HLD_TRIGGER:
			...
			return false;  // prevent key from being typed
	...
	// Repeat logic
	if (record->event.pressed) {
        if (waiting_for_repeat_key) {
            repeat_keycode = keycode;
            repeat_timer = timer_read();
            is_repeating = true;
            waiting_for_repeat_key = false;
            return true;
        } else if (is_repeating && keycode == repeat_keycode) {
            is_repeating = false;
            return false; // suppress repeated key from normal typing
        }
    }
	// Hold logic
    if (record->event.pressed) {
        if (waiting_for_hold_key) {
            held_keycode = keycode;
			register_code16(held_keycode);
            is_holding = true;
            waiting_for_hold_key = false;
            uprintf("[HOLD-LOCK] Holding keycode: %u\\n", held_keycode);
			return false;
        } else if (is_holding && keycode == held_keycode) {
			unregister_code16(held_keycode);
            is_holding = false;
			held_keycode = KC_NO;
			uprintf("[HOLD-LOCK] Holding keycode: %u\\n", held_keycode);
...
void matrix_scan_user(void) {
    if (is_repeating && timer_elapsed(repeat_timer) > 100) {
        tap_code16(repeat_keycode);
        repeat_timer = timer_read();
    }
